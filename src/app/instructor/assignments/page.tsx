...
export default function InstructorAssignmentsPage() {
    ...
    const fetchSubmissions = async () => {
        ...
        const { data, error } = await supabase
            ...
            `)
            .eq('project.course.instructor_id', session.user.id)
            ...
            try {
                if (gradingSubmission.user?.email) {
                    ...
                    if (!response.ok) {
                        ...
                        // If it's the domain verification error, we don't want to alert the user every time
                        // but we log it as a warning so they know why students aren't getting emails.
                    }
                }
            } catch (err) {
    ...
    return (
        <div style={{ padding: '2rem' }}>
            <div style={{ marginBottom: '2rem' }}>
                <Link href="/instructor" style={{ color: '#64748b', fontSize: '0.875rem' }}>← Back to Dashboard</Link>
